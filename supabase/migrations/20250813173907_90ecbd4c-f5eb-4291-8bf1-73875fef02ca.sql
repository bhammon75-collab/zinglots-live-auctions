...
create unique index if not exists uq_email_jobs_once on app.email_jobs(type, lot_id, user_id, coalesce(window,'-'));
...
returns int language sql stable set search_path=app, public as $$
  select count(*) from app.lot_watches where lot_id=p_lot;
$$;