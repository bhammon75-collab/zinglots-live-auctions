...
export function useLotRealtime(lotId: string) {
  ...
  useEffect(() => {
    ...
    // Initial fetch
    (async () => {
      const { data: lot } = await sb.from('app.lots').select('status, ends_at').eq('id', lotId).maybeSingle();
      const { data: bid } = await sb.from('app.bids').select('amount').eq('lot_id', lotId).order('amount', { ascending: false }).limit(1).maybeSingle();
